...
import { Tag,Avatar,Tabs,Row,Form,TimePicker,Table,Dropdown,Menu,Button } from 'antd';
import { InfoCircleOutlined,ArrowRightOutlined,ArrowLeftOutlined,EyeOutlined,CaretDownOutlined, FieldTimeOutlined, CoffeeOutlined, CommentOutlined, SoundOutlined, BellOutlined } from '@ant-design/icons';
...
export interface IRestaurantDetailsModalState{
  restaurantModel:RestaurantDto;
...
@inject(Stores.RestaurantManagerStore,Stores.CategoryStore)
@observer
export class RestaurantDetails extends AppComponentBase<any, IRestaurantDetailsModalState> {

  subscribedDate?:string =undefined;
  remainingDays?:number=undefined;

  state={
    restaurantModel:{} as RestaurantDto,
  ...
  async componentDidMount(){ 
    document.title= `${L("RestaurantDetails")} | YaCotch `;   
    try {
         if(this.props.match.params.id){
          let id:number = this.props.match.params.id;
          let restaurant = await restaurantsService.getRestaurant({id:id});
          let dishes = (await dishesService.getAll({restaurantId:id})).items;
          this.setState({restaurantModel: restaurant,dishes:dishes},async()=>{
  ...
  async updateReviewsList(maxResultCount: number, skipCount: number) {
    let result = await reviewsService.getAll({
      refType:ReviewRefType.Restaurant,
  ...
  async updateReportsList(maxResultCount: number, skipCount: number) {
    let result = await reportsService.getAll({
      refType:ReportRefType.Restaurant,
      maxResultCount:maxResultCount,
 ...
 render(){
   const {restaurantModel,reviews,reports,dishes}= this.state;
  ...
  const reportsPagination = {
   ...
   <div className="restaurant-page">
     ...
     {localization.isRTL() ? 
     <ArrowRightOutlined onClick={()=> window.location.href = '/restaurants'} />
     :
     <ArrowLeftOutlined onClick={()=> window.location.href = '/restaurants'} />
     }
     </span>
     ...
     <Tabs defaultActiveKey="1">
      ...
      <div className="detail-wrapper">
          ...
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.id : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('ArName')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.arName : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('EnName')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.enName : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('City')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.city?.text : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('PhoneNumber')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.phoneNumber : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('ManagerName')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.manager ? 
          <a onClick={() => this.openManagerDetailsModal({ id: restaurantModel.manager?.id })}>{restaurantModel.manager?.name}</a>
          : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('ArLogo')}</span>
          <span className="detail-value">{restaurantModel !== undefined ?
          <div onClick={() => this.openImageModal(restaurantModel.arLogo!, restaurantModel.name)} style={{ display: 'inline-block', cursor: "zoom-in" }}>
          <Avatar shape='square' size={50} src={restaurantModel.arLogo} />
        </div>: undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('EnLogo')}</span>
          <span className="detail-value">{restaurantModel !== undefined ?
          <div onClick={() => this.openImageModal(restaurantModel.enLogo!, restaurantModel.name)} style={{ display: 'inline-block', cursor: "zoom-in" }}>
          <Avatar shape='square' size={50} src={restaurantModel.enLogo} />
        </div>: undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('ArCover')}</span>
          <span className="detail-value">{restaurantModel !== undefined ?
          <div onClick={() => this.openImageModal(restaurantModel.arCover!, restaurantModel.name)} style={{ display: 'inline-block', cursor: "zoom-in" }}>
          <Avatar shape='square' size={50} src={restaurantModel.arCover} />
        </div>: undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('EnCover')}</span>
          <span className="detail-value">{restaurantModel !== undefined ?
          <div onClick={() => this.openImageModal(restaurantModel.enCover!, restaurantModel.name)} style={{ display: 'inline-block', cursor: "zoom-in" }}>
          <Avatar shape='square' size={50} src={restaurantModel.enCover} />
        </div>: undefined}</span>
        </div>
        
  
        <div className="detail-wrapper">
          <span className="detail-label">{L('CommercialRegisterNumber')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.commercialRegisterNumber : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('CommercialRegisterDocument')}</span>
          <span className="detail-value">{restaurantModel !== undefined ?
          <div onClick={() => this.openImageModal(restaurantModel.commercialRegisterDocument!, restaurantModel.commercialRegisterDocument)} style={{ display: 'inline-block', cursor: "zoom-in" }}>
          <Avatar shape='square' size={50} src={restaurantModel.commercialRegisterDocument} />
        </div>: undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('TotalRate')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.rate : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('FacebookUrl')}</span>
          <span className="detail-value">{restaurantModel !== undefined &&  restaurantModel.facebookUrl? restaurantModel.facebookUrl : L('NotAvailable')}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('TwitterUrl')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.twitterUrl ? restaurantModel.twitterUrl : L('NotAvailable')}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('InstagramUrl')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.instagramUrl ? restaurantModel.instagramUrl : L('NotAvailable')}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('Website')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.websiteUrl ? restaurantModel.websiteUrl : L('NotAvailable')}</span>
        </div>
        
      
        <div className="detail-wrapper">
          <span className="detail-label">{L('CreationDate')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? moment(restaurantModel.creationTime).format(timingHelper.defaultDateFormat) : undefined}</span>
        </div>
        
        <div className="detail-wrapper">
          <span className="detail-label">{L('Status')}</span>
          <span className="detail-value">
            <Tag color={restaurantModel !== undefined && restaurantModel.isActive ? 'green' : 'volcano'} className='ant-tag-disable-pointer'>
              {restaurantModel !== undefined && restaurantModel.isActive ? L('Active') : L('Inactive')}
            </Tag>
          </span>
    ...
    <TabPane
        ...
        <div className="opening-times-row">
          ...
          {
            restaurantModel && restaurantModel.openingDays && restaurantModel.openingDays.map((row: OpeningDayDto, key:number) => {
              return (
...
<BellOutlined />
      ...
      <div className="detail-wrapper">
          ...
          <span className="detail-value">{restaurantModel !== undefined ? restaurantModel.subscription?.name : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('SubscriptionDuration')}</span>
          <span className="detail-value">{restaurantModel !== undefined ? `${restaurantModel.subscription?.duration} ${L('Days')}` : undefined}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('SubscriptionFee')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.subscription?.fee !==undefined ? <ThousandSeparator number={restaurantModel.subscription?.fee} currency={L("SAR")} /> : L("NotAvailable")}</span>
        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('SubscribedDate')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.subscription?.creationTime !==undefined ? 
          moment(restaurantModel.subscription?.creationTime).format(timingHelper.defaultDateFormat) : L("NotAvailable")}</span>

        </div>
        <div className="detail-wrapper">
          <span className="detail-label">{L('ReminingDays')}</span>
          <span className="detail-value">{restaurantModel !== undefined && restaurantModel.subscription?.remainingDays !==undefined ? 
          `${restaurantModel.subscription?.remainingDays} ${L('Days')}` : L("NotAvailable")}</span>
