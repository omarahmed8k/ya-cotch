...
class UserService {
  ...
  public async userActivation(input: EntityDto) {
    let result = await http.post('api/services/app/User/Activate', input);
    return result.data;
  }
  public async userDeactivation(input: EntityDto) {
    let result = await http.post('api/services/app/User/DeActivate', input);
    return result.data;
  }
  public async resetPassword(input: ResetPasswordDto) {
    let result = await http.post('api/services/app/User/ResetPassword', input);
    return result.data;
  }

  public async checkIfGrantedPermission(permissionName: string): Promise<boolean> {
    let result = await http.get('api/services/app/User/CheckIfGrantedPermission',{ params: { permissionName } });
    return result.data.result ;
  }

  public async getAllLite(pagedFilterAndSortedRequest: PagedUserResultRequestDto): Promise<PagedResultDto<LiteEntityDto>> {
    let result = await http.get('api/services/app/User/GetAllLite', { params: pagedFilterAndSortedRequest });
    return result.data.result;
  }
